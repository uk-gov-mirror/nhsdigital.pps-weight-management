...
def magic_link_request(request: HttpRequest) -> HttpResponse:
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if _check_otp_generation_rate_limit(contact):
                ...
                return redirect("htsh:otp_verify")
    ...
    return render(request, "htsh/magic_link_request.jinja", {"form": form})
        ...
        if form.is_valid():
            ...
            if otp_flow == "signup":
                from web.views import (
                    ...
                    _persist_questionnaire_answer,
                    _persist_to_user_filter,
                )

                for key in QUESTIONNAIRE_SESSION_KEYS:
                    val = request.session.get(key)
                    if val is not None:
                        _persist_questionnaire_answer(request.user, key, val)
            ...
            try:
                ...
                for key in ["motivation", "priority_behaviour", "past_barriers", "current_barriers", "confidence_readiness", "enablers"]:
                ...
                pass
            # Hydrate filter keys + allow_check_in from UserFilter
            try:
...
def landing(request: HttpRequest) -> HttpResponse:
    ...
    def _set_no_store(resp: HttpResponse) -> HttpResponse:
        # Campaign-entry pages set session state; never allow shared edge caching.
    ...
    if cc:
        ...
        try:
            ...
            if campaign.is_valid_today():
                # Valid campaign entry should always start a fresh campaign journey.
                ...
                # Store campaign code in the fresh session and start onboarding.
                request.session["campaign_code"] = cc
                return _set_no_store(redirect("/"))
            else:
    ...
    response = render(request, "htsh/landing.jinja", context)
    if cc:
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if preferred_contact_method in ["email", "sms"]:
                ...
                return redirect("htsh:campaign_contact_info")
        return render(
            request,
            "htsh/campaign_contact_type.jinja",
            {"error": "Please select a contact method"},
        )
    return render(request, "htsh/campaign_contact_type.jinja")
...
def campaign_contact_info(request: HttpRequest) -> HttpResponse:
    ...
    if "preferred_contact_method" not in request.session:
        return redirect("htsh:campaign_contact_type")
    ...
    if request.method == "POST":
        form = (
            ...
            if preferred_contact_method == UserProfile.CONTACT_EMAIL
        ...
        if form.is_valid():
            ...
            phone = (
                ...
                if preferred_contact_method == UserProfile.CONTACT_SMS
                else ""
            )
            ...
            if _check_otp_generation_rate_limit(contact_for_limit):
                ...
                return render(
                    request,
                    "htsh/campaign_contact_info.jinja",
                    {
    ...
    return render(
        ...
        "htsh/campaign_contact_info.jinja",
        {
            ...
            "data": data,
            "preferred_contact_method": preferred_contact_method,
            "back_href": reverse("htsh:campaign_contact_type"),
        },
    )
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if disclaimer_accepted == "accepted":
                ...
                return redirect("htsh:details_not_shared")
        return render(
            request, "htsh/disclaimer.jinja", {"form": form, "error": True}
        )
    else:
        form = DisclaimerForm()

    return render(request, "htsh/disclaimer.jinja", {"form": form})
...
def details_not_shared(request: HttpRequest) -> HttpResponse:
    ...
    if request.session.get("disclaimer_accepted") == False:
        return render(request, "htsh/details_not_shared.jinja")
    return redirect("landing")


def account(request: HttpRequest) -> HttpResponse:
    """Show and edit the currently-logged-in user's account details."""
    user = request.user
    profile = getattr(request, "htsh_profile", None)
    if profile is None:
        profile = UserProfile.objects.get(user=user)
    ...
    if request.method == "POST":
        form = AccountForm(user=user, profile=profile, data=request.POST)
        if form.is_valid():
            form.save()
            messages.success(request, "Account updated.")
            return redirect(next_url or reverse("htsh:account"))
    else:
        form = AccountForm(user=user, profile=profile)
...
def delete_account(request: HttpRequest) -> HttpResponse:
    """Confirm + delete the user's account and related htsh records."""
    user = request.user
    profile = getattr(request, "htsh_profile", None)
    if profile is None:
        profile = UserProfile.objects.get(user=user)
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if is_returning_user == "returning":
                return redirect("htsh:magic_link_request")
            elif is_returning_user == "first-time":
                return redirect("htsh:disclaimer")
            else:
                return render(
                    request,
                    "htsh/returning.jinja",
                    {
                        "error": True,
                    },
                )
        return render(
            request, "htsh/returning.jinja", {"form": form, "error": True}
        )
    else:
        form = ReturningForm()

    return render(request, "htsh/returning.jinja", {"form": form})
...
def change_contact_info(request: HttpRequest) -> HttpResponse:
    ...
    preferred_contact_method = (
        profile.preferred_contact_method or UserProfile.CONTACT_EMAIL
    )
    errors = {}
    data = {
    ...
    return render(
        ...
        "htsh/campaign_contact_info.jinja",
        {
            ...
            "errors": errors,
            "data": data,
            "back_href": reverse("htsh:account"),
        },
    )
