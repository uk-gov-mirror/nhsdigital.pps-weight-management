"""Tests for pilot_access account management views."""
...
from testing.helpers import make_magic_link, make_pilot_profile, make_user
...
class MagicLinkRequestViewTests(TestCase):
    ...
    def test_magic_link_post_rate_limited_contact(self, mock_timing):
        ...
        cache.set("pilot_access:otp_gen:test@example.com", 3, timeout=3600)
        response = self.client.post(
            reverse("pilot_access:magic_link_request"),
            {"contact": "test@example.com"},
        )
...
class LogoutViewTests(TestCase):
    ...
    def test_logout_post_redirects_to_landing(self):
        """POST /pilot/logout/ logs out and redirects to landing."""
        self.client.force_login(self.user)
        response = self.client.post(reverse("pilot_access:logout"))
        self.assertRedirects(
            response,
    ...
    def test_logout_get_not_allowed(self):
        """GET /pilot/logout/ returns 405 (require_POST)."""
        self.client.force_login(self.user)
        response = self.client.get(reverse("pilot_access:logout"))
...
class DeleteAccountViewTests(TestCase):
    ...
    def test_delete_account_get_renders(self):
        """GET /pilot/account/delete/ returns 200."""
        response = self.client.get(reverse("pilot_access:delete_account"))
    ...
    def test_delete_account_post_deletes_and_redirects(self):
        ...
        response = self.client.post(
            reverse("pilot_access:delete_account"),
            {"confirm": True},
        )
        self.assertRedirects(
            response,
