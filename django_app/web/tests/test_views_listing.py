...
class AccountPromptInterstitialTests(_AnonymousCampaignTestCase):
    ...
    def test_create_account_redirects_to_contact_type(self):
        """POST /detail/1 action=create_account → 302 to /pilot/contact-type/."""
        response = self.client.post(
            reverse("detail", kwargs={"service_id": 1}),
            {"action": "create_account"},
        )
        self.assertEqual(response.status_code, 302)
        self.assertIn("/pilot/contact-type/", response.url)
        self.assertTrue(self.client.session.get("disclaimer_accepted"))
